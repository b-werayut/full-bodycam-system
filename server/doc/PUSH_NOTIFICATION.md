...
| Token CRUD | `src/modules/users/users.repository.js` | `upsertUserDevice` / `listActiveDeviceTokens` / `deactivateDeviceTokens` / `deactivateUserDevice` |
...
## 8. Configuration (ENV)
ทั้งคู่ optional มี default — ดู `.env.example`
...
- เรียก `/device-token` ทุกครั้งที่ได้ token ใหม่จาก `onTokenRefresh` (ถ้า access token หมดอายุ ให้ `/refresh` ก่อน)
