import { Eye, X, Calendar, Clock, MapPin, User, Video, Edit, AlertTriangle, Trash2, CheckCircle, PlayCircle, StopCircle, XCircle, Check, Copy, Plus, Save, ChevronDown, FileText, Search } from 'lucide-react';
...
// long enough that scrolling alone is painful (e.g. locations). It keeps native
// form validation working through a hidden, required input so the surrounding
// <form> still blocks submit and shows the localized "required" bubble.
function SearchableSelect({
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      {open && !disabled && (
        <div className={`absolute left-0 right-0 top-full z-30 mt-1 overflow-hidden rounded-md border shadow-lg ${darkMode ? 'border-slate-700 bg-slate-800' : 'border-slate-200 bg-white'}`}>
          <div className={`border-b p-2 ${darkMode ? 'border-slate-700' : 'border-slate-100'}`}>
                ...
                <div className={sectionClass(darkMode)}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="md:col-span-1">
                      ...
                      <SearchableSelect
                        value={formData.officerId === '' ? '' : String(formData.officerId)}
                        options={officers.map((o) => ({
                          value: String(o.officerId),
                          label: o.officerName,
                        }))}
                        onChange={(officerId) => {
                          const selected = officers.find(o => String(o.officerId) === officerId);
                          setFormData({ ...formData, officerId: selected ? selected.officerId : '', officerName: selected?.officerName || '' });
                        }}
                        placeholder={translations.selectOfficer}
                        searchPlaceholder={language === 'th' ? 'ค้นหาผู้รับผิดชอบ...' : 'Search officer...'}
                        emptyText={language === 'th' ? 'ไม่พบรายชื่อ' : 'No officers found'}
                        required
                        requiredMessage={requiredSelectMessage}
                        darkMode={darkMode}
                        triggerClassName={`${selectClassName} cursor-pointer`}
                        chevronClassName={selectChevronClassName}
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <SearchableSelect
                        ...
                        options={locationOptions.map((l) => ({
                          value: getLocationCode(l),
                          label: getLocationName(l) || getLocationCode(l),
                          keywords: getLocationCode(l),
                        }))}
                        onChange={(code) => {
                          const selected = locations.find(l => getLocationCode(l) === code);
                        ...
                        placeholder={translations.locationName}
                        searchPlaceholder={language === 'th' ? 'ค้นหาสถานที่...' : 'Search location...'}
                        emptyText={language === 'th' ? 'ไม่พบสถานที่' : 'No locations found'}
                        required
                        requiredMessage={requiredSelectMessage}
                        darkMode={darkMode}
                        triggerClassName={`${selectClassName} cursor-pointer`}
